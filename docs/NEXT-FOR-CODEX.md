...
| UX baseline | `horizontalScroll: false` · เป้ากดเล็กกว่า 44px: **36** · ตัวหนังสือ < 12px: **66** |
...
### 1. ~~แท็บ 21 อัน → 5 + More~~ ✅ เสร็จแล้ว
มือถือเหลือ **Shift · POS · Stock · CRM · More** ป้ายชื่อ 11px ทุกช่องสูง ≥44px
เดสก์ท็อปเหมือนเดิม · `MOB_PRIMARY` ใน `pos/app.fixed.jsx` คือรายชื่อ 4 แท็บนั้น

> ระหว่างแก้พบว่า**แถบแท็บไม่ใช่ตัวเดียวที่ล้น** — แถบหัว (เลือกสาขา + เลือกภาษา)
> กว้างถึง 686px ด้วย แก้โดยให้แถวห่อบรรทัดบนมือถือแทนที่จะดันหน้าจอ
> ถ้าไม่ได้วัดซ้ำจะคิดว่าแก้เสร็จแล้วทั้งที่ยังเลื่อนได้อยู่

- ล็อกไว้ด้วย `node pos/__tests__/tabbar.test.mjs` (ต้องเสิร์ฟผ่าน HTTP ก่อน)
- ผลวัดหลังแก้: `horizontalScroll: false` · `docWidth 390` · ปุ่มเล็ก 47→**36** · ตัวหนังสือเล็ก 151→**66**
